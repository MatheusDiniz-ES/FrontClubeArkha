...
import DecimalInput, { Input } from "@/components/Input";
...
const schema = yup.object({
  ...
  DataInicio: yup.string().required('Selecione a data de inicio'),
  DataFim: yup.string().required('Selecione a data de fim'),
...
export default function CadastroPromocoes() {
  ...
  const { mutate: save, isLoading: sending } = useMutation({
    mutationFn: async (values: PromocaoSendRequest) => {
      await api.post("/Promocao", {
        nome: values.NamePromotion,
        dataInicio: values.DataInicio,
        dataFim: values.DataFim,
        descricao: values.Descricao || "",
        // lojaIds: selectsLojistas.map((item) => {
        //   return Number(item.value)
        // }),
        pontuacao: pontuacao,
        banners: file64.map((file) => {
          return {
            arquivo: file
          }
        }),
        documentos: documentos64.map((file) => {
          return {
            documento: file
          }
        })
    ...
    onSuccess() {
      showDialog({
        ...
        subtitle: "O cadastro da prêmio foi concluído com êxito.",
        button: {
          testID: "ok",
          children: "Ok",
          onClick: () => router.push("/backoffice/promocoes"),
        },
      });
    },
    onError(data: any) {
      console.log(data.response.data.description)
      const description = data.response.data.description|| "Ocorreu um erro inesperado. Por favor, tente novamente mais tarde.";
      showAlert({
    ...
    <main className="flex flex-col gap-4 items-center">
      ...
      <div className="w-full flex flex-col gap-4 bg-white dark:bg-gray-750 rounded-lg border border-gray-200 dark:border-gray-600 p-8 tall:p-4">
        <div className="flex flex-col gap-0">
          ...
          <Subtitle size="xs">A informação principal da Prêmio.</Subtitle>
        </div>
