...
interface TableLojistasListProps {
  isLoading: boolean;
  sorting?: {
    field: string;
    direction: string;
...
export function TableLojistasList({
  isLoading,
  setSorting,
  sorting,
  data,
  ...
  filters,
  setFilters,
}: TableLojistasListProps) {

  const { dialog, hideDialog, showDialog } = useDialog();
  const { alert, showAlert, hideAlert } = useAlert();


  const columns: ColumnDef<Lojistas>[] = [
    {
      accessorKey: "id",
      header: ({ column }: any) => {
        return (
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Status
          variant={row.original.status == 'Ativo' ? "active" : "inactive"}
        >
      ...
      cell: ({ row }) => (
        <Popover.Root>
          ...
          <Popover.Portal>
            ...
            >
              <Link
                href={`/backoffice/lojistas/${row.original.id}`}
                className={clsx(
